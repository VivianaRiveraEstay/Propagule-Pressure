...
%% Impulso
frec_propag = 10;     % Frecuencia del prpágulo
...
tamano_propag = 10 * rand;  % Número aleatorio en (0,10)

valor_rasgo_propag = 0.4;
...
% Gráfica 
figure (1);
plot(tiempo_acumulado, Y_acumulado(:, 1), '-b', 'LineWidth', 2, 'DisplayName', 'Presa'); % Línea azul más gruesa
hold on;
plot(tiempo_acumulado, Y_acumulado(:, 2), '-g', 'LineWidth', 2, 'DisplayName', 'Depredador nativo'); % Línea verde más gruesa
plot(tiempo_acumulado, Y_acumulado(:, 3), '-r', 'LineWidth', 2, 'DisplayName', 'Depredador exótico'); % Línea roja más gruesa
legend;
xlim([0, 8000]);
ylim([0, 40]);
xlabel('Tiempo');
